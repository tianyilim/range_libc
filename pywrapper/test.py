...
for x in range(10):
    ...
    def bench(obj, name):
        ...
        dur_np = end - start
        print(",,,"+name+" np: finished computing",
              ranges.shape[0], "ranges in", dur_np, "sec")
        start = time.clock()
        ...
        if diff > 0.001:
            print(
                ",,,"+"Numpy result different from slow result, investigation possibly required. norm:", diff)
        # print("DIFF:", diff)

        print(",,,"+name+": finished computing",
              ranges.shape[0], "ranges in", dur, "sec")
