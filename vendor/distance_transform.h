...
/// in memory, this class wraps it providing a multi-dimensional matrix interface. It is possible to
...
class MArray {
   public:
    ...
    /**
     *    @brief Reorders the sub-matrixes s.t. the one at 0 <= i < D goes to 0 <= order[i] < D,
     * i.e. m[*]..[*]_i...[*] swaps with m[*]...[i]...[*]_order[i]...[*].
   ...
   private:
    ...
    std::size_t _offset[D];  ///< Jumps' offsets from the beginning of a "row" to the beginning of
                             ///< the next one, for each dimension.
};
   ...
   public:
    ...
    inline void window(const std::size_t start, const std::size_t size, MArray<T, 1> &p)
    {
        ...
        if (start + size > _size[0]) {
            std::stringstream stream;
            stream << "Window size " << size << " is out of range [" << 0 << ", "
                   << _size[0] - start << ']';
            throw std::out_of_range(stream.str());
        }
    ...
    /**
     *    @brief Gives the total offset, from the beginning of the stored array, of the i-th
     * element.
     *    @param i                  The element whose offset is requested.
...
class MMArray : public MArray<T, D> {
   public:
    ...
    MMArray &operator=(const MMArray &mma)
    {
        if (&mma != this) {
            ...
            mma.size(size);
            resize(size);
            std::memcpy(_arrayPtr.get(), mma._arrayPtr.get(),
                        MArray<T, D>::totalSize() * sizeof(T));
        }
        return *this;
    }
   ...
   private:
    std::unique_ptr<T[]> _arrayPtr;  ///< Sharp pointer in memory to this matrix, with automatic
...
class DistanceTransform {
   public:
    /**
     *    @brief Compute the L2-norm distance field D of a DIM-dimensional sampled function f. D
     * gets the distance from the local minima of f.
    ...
    inline static void distanceTransformL2(const MMArray<Scalar, 1> &f, MMArray<Scalar, 1> &D,
                                           const bool squared = false)
    {
        std::size_t size[1];
        f.size(size);
    ...
    /**
     *    @brief Compute the L2-norm distance field D of a DIM-dimensional sampled function f. D
     * gets the distance from the local minima of f. Compute also the (index of the) nearest local
    ...
    /**
     *    @brief Compute the L2-norm distance field D of a 1-dimensional sampled function f. D gets
     * the distance from the local minima of f. Compute also the (index of the) nearest local
     * minimum of each sample.
     *    @param f              A 1-dimensional, regularly sampled function.
     *    @param D              The resulting distance field of f.
     *    @param I              Resulting array containing the (index of the) local minimum for each
    ...
    template <typename Scalar = float>
    inline static void distanceTransformL2(const MMArray<Scalar, 1> &f, MMArray<Scalar, 1> &D,
                                           MMArray<std::size_t, 1> &I, const bool squared = false)
    {
        std::size_t size[1];
        f.size(size);
        if (&D != &f) D.resize(size);
   ...
   private:
    ...
    inline static void distanceL2(const MArray<Scalar, 1> &f, MArray<Scalar, 1> &D)
    {
        ...
        for (std::size_t q = 1; q < f.size(); ++q) {
            ...
            do {
                --k;
                // compute horizontal position of intersection between the parabola from q and the
                // current lowest parabola
    ...
    template <typename Scalar = float>
    inline static void distanceL2(const MArray<Scalar, 1> &f, MArray<Scalar, 1> &D,
                                  const MArray<std::size_t, 1> &Ipre, MArray<std::size_t, 1> &Ipost)
    {
        if (f.size() == 0 || f.size() > D.size()) return;
        if (f.size() == 1) {
            D[0] = f[0];
